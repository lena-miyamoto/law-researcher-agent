#!/usr/bin/env python3
"""Validate structural and data integrity of a law-db archive.
...
checks also run automatically at the end of every script that modifies law-db/.
"""
...
def _format_human(findings):
    ...
    if not findings:
        lines.append("✓ law-db integrity check passed — no issues found.")
        return "\n".join(lines)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Validate structural and data integrity of a law-db archive.",
    )
    parser.add_argument(
        "--law-db",
        default="law-db",
...
def main():
    args = parse_args()
    root = Path(args.law_db)

    if not root.is_dir():
        print(f"ERROR: law-db directory not found: {root}", file=sys.stderr)
        if args.json_output:
            print(
                json.dumps(
                    {
                        ...
                        "findings": [
                            finding(
                                ...
                                "Create the directory or check the --law-db path.",
                            )
                        ],
                    },
                    indent=2,
